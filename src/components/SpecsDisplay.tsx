...
const DetailItem: React.FC<{ label: string; value?: string | string[] | number | {value: number | string, unit: string} }> = ({ label, value }) => {
  ...
  return (
    <li className="flex flex-col sm:flex-row sm:justify-between py-2 border-b border-[var(--llyc-gray-4)] last:border-b-0">
      <span className="font-semibold text-[var(--llyc-gray-2)] font-open-sans">{label}:</span>
      <span className="text-[var(--llyc-dark-blue)] text-left sm:text-right font-open-sans">{displayValue}</span>
    </li>
  );
};
...
export const SpecsDisplay: React.FC<SpecsDisplayProps> = ({ spec }) => {
  ...
  return (
    <section className="bg-[var(--llyc-card-bg)] p-6 rounded-xl shadow-md my-8 transition-all duration-300 hover:shadow-lg hover:shadow-[var(--llyc-turquoise)]/30 border border-[var(--llyc-gray-4)]">
      <h2 className="text-2xl font-montserrat font-semibold mb-4 text-[var(--llyc-turquoise)] border-b-2 border-[var(--llyc-turquoise)] pb-2">
        {t('specsDisplay.titlePrefix')} <span className="text-[var(--llyc-dark-blue)] font-normal">{translatedFormatName} {t('specsDisplay.forPlatform')} {translatedPlatform}</span>
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
        ...
        <div>
          ...
          {spec.bestPracticesKeys.length > 0 ? (
            <ul className="list-disc list-inside text-sm text-[var(--llyc-gray-1)] space-y-1 font-open-sans">
       ...
       {spec.promptGuidanceKey && (
          <div className="mt-4 pt-3 border-t border-[var(--llyc-gray-4)]">
